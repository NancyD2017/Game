...
public class ExchangeWithPlayer {
    ...
    void act(Player pl) {                                                              //осуществляет обмен игрока с другими игроками
        ...
        while (exchanger == null)
            exchanger = (Integer) StringCatcher.getData("org.example.controller.Buttons_EPlayer");
        if (!exchanger.equals(0)) {
            ...
            while (!isAllRight) {
                ...
                if (!playerGives.contains("0")) {
                    ...
                    if (!Arrays.toString(playerReceives).contains("0")) {
